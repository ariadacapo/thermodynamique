...
\onlyframabook{\clearpage}%handmade: pour ne pas débuter cette section-clé tout en bas de la page
...
\label{ch_premier_principe_sf}
	...
	\end{equation}
	\onlyframabook{\begin{small}}%handmade
	\begin{equationterms}
...
\section[Quantifier le travail avec un système fermé]{Quantifier le travail avec un système\onlyamphibook{\\} fermé}
	...
	\label{ch_travail_pdv}
		...
		Un outil absolument formidable pour comprendre et analyser les transferts de chaleur est le \vocab{diagramme pression-volume}.\index{pression-volume!diagramme} Dans le cas où l’on modélise le fluide par un ressort, le travail peut être visualisé par l’aire sous la courbe d’une évolution (\cref{fig_p-v_ressort}).		
		...
		\begin{figure}
			...
			Cette figure représente le même phénomène qu’en \cref{fig_force-déplacement-aire} avec des grandeurs différentes.}{schéma \cczero \oc}
	...
	\subsection{Travail d’un fluide en évolution lente}
		...
		\thermoquoteend{François-Marie Guyonneau de~Pambour, 1839}{\textit{Théorie de la machine à vapeur} \cite{pambour1839}\onlyamphibook{\vspace{4em}}\onlyframabook{\vspace{1em}}} %handmade vspace
...
Plus on lui apporte de chaleur pendant la compression, plus la pression augmente fortement. La courbe adiabatique représente le cas où aucun apport ni perte de chaleur n’a lieu ($Q = 0$).}{schéma \cczero \oc}
		...
		\onlyframabook{\clearfloats}%handmade : poser les flottaisons sinon les exemples sont explosés
			\begin{anexample}
	...
	\label{ch_reversibilite}
		...
		\thermoquoteend{Richard Feynman, 1963}{\textit{The Feynman Lectures on \mbox{Physics}} \mbox{\cite{feynman1963, feynman1963fr}}\onlyamphibook{\vspace{-2em}}} %handmade vspace
