...
\wherefrom{[DS n°2 2012, 11pts]}\index{Carnot!cycle de}\index{cycle!de Carnot}
% Cet exo est lié à l’exercice 8.5 (\ref{exo_diagrammes_ts})
...
\titreresultats
	...
	\begin{description}
		...
		\item [\ref{exo_efficacite_refrigerateur_carnot}]
					...
					\tab 2) Compressions et détentes non réversibles (ainsi $w_{4\to 3} + w_{2\to 1} > 0$), en particulier si une soupape est utilisée (\S\ref{ch_principe_fonctionnement_réfrigérateur} p.\pageref{ch_principe_fonctionnement_réfrigérateur}) ; transferts de chaleur non-isothermes.
		\item [\ref{exo_efficacite_thermopompe_carnot}]
					\tab 1) cf. \S\ref{ch_rendement_thermopompe}	p.\pageref{ch_rendement_thermopompe} ;
		...
		\item [\ref{exo_cours_carnot}]
					\tab 1) cf. \S\ref{ch_descritpion_cycle_carnot} p.\pageref{ch_descritpion_cycle_carnot}, et en particulier les figures~\ref{fig_carnot_quatre_etapes} et~\ref{fig_carnot_quatre_etapes_so} ;
					\tab 2) cf. \S\ref{ch_elaboration_cycle_carnot} p.\pageref{ch_elaboration_cycle_carnot} ;
		...
		\item [\ref{exo_moteur_turbine_ideal}]
					...
					\tab 1) C’est l’agencement représenté en \cref{fig_carnot_quatre_etapes_so} p.\pageref{fig_carnot_quatre_etapes_so} ;
