\index{principes de la thermodynamique!second|(textbf}
\section{Le second principe}

	\subsection{Énoncé}
	\label{ch_second_principe_enonce}

		Le \vocab[principes de la thermodynamique!second, énoncé|textbf]{second principe de la thermodynamique} s’exprime ainsi :
		...
		\index{chaleur!transfert vers une température plus haute}
		\begin{trucimportant}
...
\label{ch_second_principe_machines_thermiques}
	...
	\label{ch_demo_second_principe}\index{efficacité!maximale des machines}\index{moteur!principe de fonctionnement}\index{cycle!thermodynamique, faiblesse du rendement}
		...
		\thermoquoteend{Sadi Carnot, 1824~\cite{carnot1824}\index{Carnot!Sadi}}{\onlyframabook{\vspace{2em}}}%handmade
	...
	\label{ch_limites_machines_thermiques}
		...
		\begin{description}
			...
			\index{principes de la thermodynamique!second, appliqué aux moteurs}\index{moteur!rendement maximal d’un}\index{moteur!principe de fonctionnement}
		...
		\clearfloats\index{principes de la thermodynamique!second, appliqué aux réfrigérateurs (ou climatiseurs)}\index{refrigerateur@réfrigérateur!rendement maximal d’un}\index{efficacité!d’un réfrigérateur}\index{efficacité!maximale d’un réfrigérateur}\index{climatiseur!rendement maximal d’un}\index{efficacité!d’un climatiseur}\index{efficacité!maximale d’un climatiseur}\index{efficacité!d’une pompe à chaleur}\index{efficacité!maximale d’une pompe à chaleur}\index{pompe à chaleur!rendement maximal d’une}\index{cycle!de réfrigération (ou climatisation)}
...
\label{ch_cycle_de_carnot}\index{Carnot!cycle de|(textbf}\index{cycle!de Carnot|(textbf}
	...
	\label{ch_contexte_cycle_carnot}
		...
		\index{Carnot!Sadi}\index{moteur!rendement maximal d’un}
	...
	\label{ch_concept_machine_reversible}\index{reversibilite@réversibilité}\index{reversibilite@réversibilité!réversible vs. inversable}\index{inversable vs. réversible}

		\index{Carnot!Sadi, raisonnement de}
	...
	\label{ch_elaboration_cycle_carnot}
		...
		\index{evolutions@évolutions!isentropiques}\index{isentropique(s)!évolutions}\index{evolutions@évolutions!adiabatiques réversibles}\index{adiabatiques!réversibles, évolutions}\index{evolutions@évolutions!isothermes}\index{isothermes, évolutions}
		\begin{enumerate}
	...
	\label{ch_descritpion_cycle_carnot}
		...
		\begin{itemize}
			...
			\item \textbf{Compression adiabatique réversible de 1 à 2}
				\index{evolutions@évolutions!isentropiques}\index{isentropique(s)!évolutions}\index{evolutions@évolutions!adiabatiques réversibles}\index{adiabatiques!réversibles, évolutions}
			...
			\item \textbf{Chauffage isotherme de 2 à 3}
				\index{evolutions@évolutions!isothermes}\index{isothermes, évolutions}
			...
			% Baby one more time
			\renewcommand\labelitemi{\includegraphics[width=3cm]{images/illustration_adiabatique.png}}
			\item \textbf{Détente adiabatique réversible de 3 à 4}
				\index{evolutions@évolutions!isentropiques}\index{isentropique(s)!évolutions}\index{evolutions@évolutions!adiabatiques réversibles}\index{adiabatiques!réversibles, évolutions}
		...
		\thermoquoteend{Émile Clapeyron, 1834~\cite{clapeyron1834}\index{Clapeyron!Benoît Paul Émile}}{\onlyframabook{\vspace{-2em}}}
	...
	\subsection{Quatre étapes ou quatre temps ?}
		...
		Les moteurs à pistons sont souvent classés selon leur mode de fonctionnement. Les moteurs à \vocab[moteur!a@à quatre temps]{quatre temps}\index{temps moteur!quatre temps} (\cref{fig_quatre_temps} effectuent une détente tous les deux tours. La distinction porte sur le mode de vidange des gaz brûlés et de leur remplacement par de l’air frais (\textit{cf.} \S\ref{ch_deux_temps} p.\pageref{ch_deux_temps}).
	...
	\subsection{Le réfrigérateur de Carnot}
		...
		\end{figure}
\index{cycle!de Carnot|)textbf}\index{Carnot!cycle de|)textbf}%whew

\section{L’échelle de température thermodynamique}
\label{ch_echelle_temperature_thermodynamique}
\index{température!echelle@échelle de|(textbf}\index{température!thermodynamique|(textbf}
	...
	\subsection{Qu’est-ce qu’une échelle en physique ?}
		...
		\index{température!zéro}\index{echelle@échelle (physique)}\index{propriété physique}
	...
	\label{thermometres_celsius_fahrenheit}
		...
		\index{Celsius!Anders|(}\index{Celsius!température selon|(}\index{température!selon Celsius|(}\index{Fahrenheit!Daniel Gabriel|(}\index{Fahrenheit!température selon|(}\index{température!selon Fahrenheit|(}
		...
		\index{echelle@échelle (physique)}
	...
	\subsection{Le thermomètre de William Thomson}
		...
		\thermoquoteend{William Thomson, 1854~\cite{kelvin1854}\index{Kelvin!William Thomson}}{\onlyframabook{\vspace{-3em}}}
	...
	\index{température!zéro}\index{Kelvin!température selon}\index{température!selon Kelvin}\index{Celsius!température selon}\index{température!selon Celsius}%
		...
		\thermoquoteend{William Thomson, 1854~\cite{kelvin1854}\index{Kelvin!William Thomson}}{\onlyframabook{\vspace{-1em}}}%handmade
	...
	\index{température!echelle@échelle de|)textbf}\index{température!thermodynamique|)textbf}%whew

\section{Efficacité maximale des machines}
\label{ch_efficacite_maximale_machines}\index{efficacité!maximale des machines|(}
	...
	\label{ch_efficacité_moteur_carnot}\index{efficacité!d’un moteur}\index{efficacité!maximale d’un moteur}\index{moteur!rendement maximal d’un|textbf}\index{température!maximale d’un moteur}\index{température!minimale d’un moteur}\index{moteur!température minimale d’un}

		\thermoquotebegin{O}
			Ainsi nous sommes conduits à établir la proposition générale que voici :
\emph{La puissance motrice de la chaleur est indépendante des agens mis en œuvre pour la réaliser ; sa quantité est fixée uniquement par les températures des corps entre lesquels se fait en dernier résultat le transport du calorique}.\nolinebreak\makebox(18,5){\color{gray}\scalebox{2}{»}}\par\vspace{-0.3cm}\begin{flushright}Sadi Carnot, 1824\index{Carnot!Sadi}~\cite{carnot1824}\end{flushright}
		...
		\thermoquoteend{Rudolf Diesel, 1893~\cite{diesel1893, diesel1893en}\index{Diesel!Rudolf}}{\onlyframabook{\vspace{-3em}}}
		...
		\index{efficacité!d’un moteur}\index{efficacité!maximale d’un moteur}\index{moteur!rendement maximal d’un}\index{température!maximale d’un moteur}\index{température!minimale d’un moteur}\index{moteur!température minimale d’un}
	...
	\label{ch_efficacite_refrigerateur_carnot}\index{efficacité!d’un réfrigérateur}\index{efficacité!maximale d’un réfrigérateur}\index{efficacité!d’un climatiseur}\index{efficacité!maximale d’un climatiseur}\index{climatiseur!rendement maximal d’un}\index{refrigerateur@réfrigérateur!rendement maximal d’un}
	...
	\index{efficacité!d’une pompe à chaleur}\index{efficacité!maximale d’une pompe à chaleur}\index{pompe à chaleur!rendement maximal d’une}
		...
		\end{equation}
		\begin{equationterms}\onlyframabook{\vspace{-0.5cm}}%handmade pour clôturer le chapitre sur cette page
		...
		\begin{anexample}
		 	Une pompe à chaleur est utilisée pour chauffer de l’eau à~\SI{120}{\degreeCelsius} dans un environnement à~\SI{-5}{\degreeCelsius}. Quel est le rendement maximal atteignable ?
		 		...
		 		\end{answer}
		 \end{anexample}
	 \index{efficacité!maximale des machines|)}
\index{principes de la thermodynamique!second|)textbf}
