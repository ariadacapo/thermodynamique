\index{cycles!moteur à gaz|(}\index{moteur!cycles à gaz|(}
...
\label{ch_criteres_evaluation_moteurs_gaz}\index{moteur!critères d’évaluation}
	...
	\label{ch_rapport_des_puissances}\index{marge de travail|(}\index{travail!marge de|(}
...
Les moteurs à mouvement alternatif (souvent dits «~à pistons-cylindres~») admettent une quantité d’air finie et effectuent leur cycle thermodynamique sur cette masse. Le cycle peut être effectué de multiples fois en parallèle, et est répété plusieurs fois dans le temps pour fournir une puissance continue (un moteur automobile effectue usuellement une cinquantaine de cycles par seconde).
	...
	\label{ch_cycle_otto}\index{Otto!cycle d’|(}\index{cycle!d’Otto|(}\index{moteur!à essence|(}
		...
		\index{évolutions!isochores}\index{isochores, évolutions}\index{isentropiques, évolutions}\index{évolutions!isentropiques}\index{adiabatiques réversibles, évolutions}\index{évolutions!adiabatiques réversibles}\index{chaleur!transferts réversibles}\index{travail!transferts réversibles}
	...
	\index{Otto!cycle d’|)}\index{cycle!d’Otto|)}\index{moteur!à essence|)}

	\subsection{Le cycle de Diesel}
	\label{ch_cycle_diesel}\index{Diesel!cycle|(}\index{cycle!de Diesel|(}
...
Le combustible, ainsi, ne doit pas être précédemment mélangé à l’air, mais ce dernier doit être comprimé séparément, sinon, bien avant que la compression requise ne soit atteinte, l’allumage se produirait, et le cycle serait interrompu.
	...
	\index{Diesel!cycle|)}\index{cycle!de Diesel|)}
...
\index{moteur!à gaz, composants des}\index{turbomachine (moteur à turbine)}\index{turbine!moteur à (turbomachine)}\index{moteur!à turbine (turbomachine}
	...
	\index{compresseur}
		...
		Tout comme nous l’avons fait pour la turbine (\ref{def_efficacité_isentropique_turbine}), nous quantifions l’efficacité d’un compresseur en comparant sa puissance avec celle d’un compresseur idéal (un compresseur qui serait isentropique). Nous nommons ce paramètre l’\vocab[efficacité!isentropique]{efficacité isentropique}\index{compresseur!efficacité isentropique}\index{isentropique, efficacité} du compresseur $\eta_\C$ :
		\begin{equation}
	...
	\index{combustion!chambre de}\index{chambre de combustion}
		...
		\index{chambre de combustion!puissance}\index{puissance!d’une chambre de combustion}
				...
				\begin{answer}
					...
					$C_\B = \left[ - 2 \ c_{p \text{(gaz)}} \ (T_\B - T_\A) - C_\A^2 \right]^{\frac{1}{2}}
							...
							= \SI{496,2}{\metre\per\second} = \SI{1786}{\kilo\metre\per\hour}$.
								\begin{remark}
									\index{irréversibilité}\index{pression!atmosphérique}\index{compressibilité des écoulements}\index{écoulements compressibles}
...
\label{ch_configurations_turbomachines}\index{turbomachine (moteur à turbine)!configurations|(}\index{moteur!à turbine (turbomachine)!configurations|(}
	...
	\index{turbomachine (moteur à turbine)!intérêt}\index{moteur!à turbine (turbomachine)!intérêt}
		...
		\index{poussée spécifique}\index{spécifique!poussée}\index{puissance!spécifique}\index{spécifique!puissance}
	...
	\label{ch_generateur_gaz}\index{générateur à gaz|(}
	...
	\label{ch_turboprop_turboshaft}\index{turbopropulseur|(}\index{turbomoteur|(}
		...
		\index{turbopropulseur|)}\index{turbomoteur|)}

	\dontbreakpage

	\subsection{Turbofan}
	\index{turbofan|(}\index{turboréacteur!à double flux|(}\index{turbofan|(}
	...
	\subsection{Turbine libre et turbines multiples}
		...
		\index{réactivité}\index{moteur!réactivité d’un}
...
\index{turbomachine (moteur à turbine)!modification des cycles|(}\index{moteur!à turbine (turbomachine)!modification des cycles|(}\index{cycle!des turbomachines, modifications|(}
	...
	\label{ch_intercooling}
		...
		\index{travail!marge de}\index{marge de travail}\index{puissance!spécifique}\index{spécifique!puissance}
		...
		\index{efficacité!thermique}\index{efficacité!d’un moteur}\index{moteur!rendement d’un}
	...
	\label{ch_postcombustion}
		...
		\index{poussée spécifique}\index{spécifique!poussée}\index{efficacité!thermique}\index{efficacité!d’un moteur}\index{moteur!rendement d’un}
	...
	\label{ch_refroidissement_turbine}\index{refroidissement turbine}\index{turbine!refroidissement}\index{prélèvement!compresseur}\index{compresseur!prélèvement}

		\index{efficacité!thermique}\index{efficacité!d’un moteur}\index{moteur!rendement d’un}\index{température!maximale d’un moteur}\index{moteur!température maximale d’un}
