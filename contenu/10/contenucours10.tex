\index{cycle!moteur à gaz|(textbf}\index{moteur!cycles à gaz|(textbf}\index{combustion!interne, moteur à|(textbf}\index{moteur!a@à combustion interne|(textbf}\index{gaz!cycles moteur|(textbf}
\section{Pourquoi utiliser un moteur à gaz ?}
	...
	\begin{itemize}
		...
		\item D’autre part, l’apport de chaleur se fait sans perte. En effet, il est possible d’effectuer une combustion \emph{directement à l’intérieur} du fluide de travail --\ c’est ce que l’on nomme la \vocab[moteur!a@à combustion interne]{la combustion interne}\index{combustion!interne, moteur à}.
...
\label{ch_criteres_evaluation_moteurs_gaz}\index{moteur!critères d’évaluation}
	...
	\label{ch_rapport_des_puissances}\index{marge de travail|(textbf}\index{travail!marge de|(}
		...
		\index{efficacité!d’une turbine}\index{turbine!efficacité isentropique}\index{efficacité!isentropique}\index{isentropique(s)!efficacité}
	...
	\subsection{Autres critères d’évaluation}
		...
		\index{ecologique@écologique, impact}\index{environnement, impact sur}
...
Les moteurs à mouvement alternatif (souvent dits «~à pistons-cylindres~») admettent une quantité d’air finie et effectuent leur cycle thermodynamique sur cette masse. Le cycle peut être effectué de multiples fois en parallèle, et est répété plusieurs fois dans le temps pour fournir une puissance continue (un moteur automobile effectue usuellement une cinquantaine de cycles par seconde).
	...
	\subsection{Intérêt des moteurs à pistons}
		...
		\index{isothermes, évolutions}\index{evolutions@évolutions!isothermes}
	...
	\label{ch_cycle_otto}\index{Otto!cycle d’|(textbf}\index{cycle!d’Otto|(textbf}
		...
		\index{Otto!Nikolaus}
		On doit à l’ingénieur allemand \wf{Nikolaus Otto}, en 1864, la mise au point du moteur que l’on connaît aujourd’hui sous le nom de «~\vocab[]{moteur à essence}~». Le cycle de principe de ce moteur, appelé \vocab[cycle!d’Otto]{cycle d’Otto}\index{Otto!cycle d’}, est constitué de deux phases isentropiques encadrées par deux phases isochores ; il est décrit en \cref{fig_cycle_otto}.
		\index{evolutions@évolutions!isochores}\index{isochores, évolutions}\index{isentropique(s)!évolutions}\index{evolutions@évolutions!isentropiques}\index{adiabatiques!réversibles, évolutions}\index{evolutions@évolutions!adiabatiques réversibles}
	...
	\label{ch_cycle_diesel}\index{Diesel!cycle|(textbf}
...
Le combustible, ainsi, ne doit pas être précédemment mélangé à l’air, mais ce dernier doit être comprimé séparément, sinon, bien avant que la compression requise ne soit atteinte, l’allumage se produirait, et le cycle serait interrompu.
		...
		Fruit du travail patient et soigneux de son inventeur, l’ingénieur allemand \wf{Rudolf Diesel} (\S\ref{ch_histoire_diesel}), le \vocab[Diesel!moteur]{moteur \mbox{Diesel}} propulse aujourd’hui l’écrasante majorité des transports commerciaux sur route et sur mer.

		\index{isobares, évolutions}\index{evolutions@évolutions!isobares}\index{isentropique(s)!évolutions}\index{evolutions@évolutions!isentropiques}\index{adiabatiques!réversibles, évolutions}\index{evolutions@évolutions!adiabatiques réversibles}
	...
	\index{Diesel!cycle|)textbf}

	\subsection{Mise en pratique des cycles}
	\label{ch_cycles_pistons_reels}\index{moteur!alternatif, cycle réel}\index{cycle!réel d’un moteur alternatif}

		\index{temps moteur!vidange}\index{vidange (temps moteur)}\index{irréversibilité!lors des transferts de travail}
		...
		\index{ecologique@écologique, impact}\index{environnement, impact sur}\index{injection directe}\index{compression, taux de}\index{taux de compression}
...
\index{turbomachine (moteur à turbine)!composants}
	...
	\index{compresseur}
		...
		\index{isentropique(s)!évolutions}\index{evolutions@évolutions!isentropiques}\index{adiabatiques!réversibles, évolutions}\index{evolutions@évolutions!adiabatiques réversibles}\index{travail!transferts réversibles}
		Tout comme nous l’avons fait pour la turbine (\ref{def_efficacité_isentropique_turbine}), nous quantifions l’efficacité d’un compresseur en comparant sa puissance avec celle d’un compresseur idéal (un compresseur qui serait isentropique). Nous nommons ce paramètre l’\vocab[efficacité!isentropique]{efficacité isentropique}\index{compresseur!efficacité d’un}\index{isentropique(s)!efficacité} du compresseur $\eta_\C$ :
		\begin{equation}
	...
	\index{combustion!chambre de}\index{chambre de combustion}
		...
		\index{isochores, évolutions}\index{evolutions@évolutions!isochores}
		...
		\index{puissance!d’une chambre de combustion}\index{gaz!d’échappement}\index{gaz parfait!modélisation des gaz brûlés}\index{gaz!brûlés, modélisation par un gaz parfait}
				...
				\begin{answer}
					...
					$C_\B = \left[ - 2 \ c_{p \text{(gaz)}} \ (T_\B - T_\A) - C_\A^2 \right]^{\frac{1}{2}}
							...
							= \SI{496,2}{\metre\per\second} = \SI{1786}{\kilo\metre\per\hour}$.
								\begin{remark}
									\index{irréversibilité!lors des transferts de travail}\index{pression!atmosphérique}\index{atmosphère, pression de l’}\index{compressibilité des écoulements}\index{ecoulement@écoulements compressibles}
...
\label{ch_configurations_turbomachines}\index{turbomachine (moteur à turbine)!configurations|(}


	\subsection{Intérêt des turbomachines}
	\index{turbomachine (moteur à turbine)!intérêt}
		...
		\index{reactivite@réactivité d’un moteur}\index{moteur!réactivité}
	...
	\label{ch_turboreacteur}\index{turboréacteur!à simple flux|(textbf}
		...
		\index{turboréacteur!à simple flux|)textbf}
	...
	\label{ch_turboprop_turboshaft}\index{turbopropulseur|(textbf}\index{turbomoteur|(textbf}
		...
		L’arbre moteur fournit alors du travail, que l’on peut utiliser pour alimenter une hélice propulsive (cas d’un \vocab{turbopropulseur}) ou un élément externe comme une génératrice ou une pompe (cas d’un \vocab{turbomoteur}), comme montré en \cref{fig_turbomoteur}. Le cycle de ces machines est parfois appelé \vocab[cycle!de Brayton]{cycle de Brayton}.\index{Brayton, cycle de}
	...
	\subsection{Turbofan}
	\index{turboréacteur!à double flux|(textbf}
	...
	\subsection{Turbine libre et turbines multiples}
		...
		\index{turbomachine (moteur à turbine)!configurations|)}



\section[Modification des cycles des turbomachines]{Modification des cycles des\onlyamphibook{\\} turbomachines}
\index{turbomachine (moteur à turbine)!modification des cycles|(}
	...
	\label{ch_intercooling}
		...
		Pour réduire la puissance consommée par le compresseur, on a parfois recours au \vocab[refroidissement!intermédiaire]{refroidissement intermédiaire} (ou \vocab{intercooling}). La compression est interrompue et l’air est refroidi avant de poursuivre la compression (\cref{fig_intercooling_réchauffe}).
		...
		Dans la même optique, on peut augmenter la puissance spécifique fournie par la turbine en procédant au réchauffement des gaz avant la fin de la détente : c’est la \vocab[rechauffe@réchauffe (turbomachine)]{réchauffe}. Le principe et le procédé sont identiques à la resurchauffe des centrales à vapeur (\S\ref{ch_resurchauffe} p.\pageref{ch_resurchauffe}).
	...
	\label{ch_refroidissement_turbine}\index{refroidissement!turbine}\index{turbine!refroidissement}\index{prélèvement!en compresseur}\index{compresseur!prélèvement}
		...
		\begin{figure}
			...
			\end{center}
			\supercaption{Schéma du circuit thermodynamique d’un turbofan moderne.\index{turboréacteur!à double flux}
