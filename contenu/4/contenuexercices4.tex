...
\label{exo_puissance_pompe_air}
	...
	Une pompe à air (\cref{fig_exo_compresseur_air}) comprime de l’air en régime continu, de façon adiabatique. L’air voit sa température augmenter de~\SI{15}{\degreeCelsius} à~\SI{100}{\degreeCelsius}.
...
\wherefrom{[Partiel S1 2012, 8pts]}
	...
	\onlyframabook{\begin{figure}[h!]}
		\begin{center}
			\onlyamphibook{\includegraphics[width=0.8\textwidth]{images/entree_genx2b.jpg}}
			\onlyframabook{\includegraphics[width=0.9\columnwidth]{images/entree_genx2b.jpg}}
		\end{center}
...
\index{Diesel!moteur}\index{Diesel!Rudolf}
	...
	Le piston au sein du cylindre fait varier périodiquement le volume entre~\SI{3}{\liter} (\vocab[point mort bas \& haut]{point mort bas}, piston en bas de sa course) et~\SI{0,3}{\liter} (\vocab[]{point mort haut}, piston en haut de sa course).
...
\wherefrom{Partiel S1 2013, 10pts}\index{turboréacteur}
	...
	\begin{figure}
		...
		\end{center}
		\supercaption{Schéma de principe d’un turboréacteur. L’air traverse la machine de gauche à droite.}{schéma \ccbysa \olivier}
...
\begin{description}
	...
	\item [\ref{exo_gp_turbine_turboreacteur}]
					\tab 1) Avec l’équation~\ref{eq_petite_sfee_deltas_h}, $w_\text{turbine} = c_p (T_\B - T_\A) + \frac{1}{2}(C_\B^2 - C_\A^2) - q_\fromatob = \SI{-275}{\kilo\joule\per\kilogram}$ 	
	...
	\item [\ref{exo_turboreacteur_simple_flux}]
						...
						\tab 3) Avec l’équation~\ref{eq_petite_sfee_deltas_h}, $w_\text{compresseur} = w_\fromatob = \SI{+338,61}{\kilo\joule\per\kilogram}$
						...
						\tab 8) Avec l’équation~\ref{eq_petite_sfee_deltas_h}, $C_\text{E} = \left(-2\Delta h\right)^\frac{1}{2} = \SI{909,98}{\metre\per\second}$\\
