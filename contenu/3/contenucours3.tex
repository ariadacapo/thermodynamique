...
\index{système!ouvert, premier principe}\index{principes de la thermodynamique!premier, système ouvert}
		...
		\begin{anexample}
			...
			Quelle est la puissance du compresseur, si ses transferts de chaleur sont négligeables ?
				\begin{answer}
					...
					\begin{comment}
						Construction de l’exemple : compression de 0,8 à 30 bar selon une polytropique pv^1,5 = k.
	...
	\subsection{L’enthalpie}
		...
		\begin{anexample}	
			Dans une tuyère, l’air est détendu sans transfert de travail ni de chaleur. Il entre avec une enthalpie spécifique de~\SI{776}{\kilo\joule\per\kilogram} et une vitesse de~\SI[per-mode = symbol]{30}{\kilo\metre\per\hour} et ressort à même altitude, avec une enthalpie de~\SI{754}{\kilo\joule\per\kilogram}.
		...
		\begin{anexample}
			...
			Quelle est la puissance consommée sous forme de travail ?
				...
				\end{answer}
					...
					\begin{remark}La puissance du compresseur est \num{190} fois plus grande que celle de la pompe de l’exemple précédent. De plus, le volume spécifique de l’air à l’entrée est~\num{800}~fois plus grand : il faudra une machine de taille beaucoup plus importante (elle doit absorber un débit volumique $\dot{V}_\A = \dot{m} \ v_\A = \SI{1,6}{\metre\cubed\per\second} = \SI{1600}{\liter\per\second}$ à l’entrée).\index{pompe}\end{remark}
		\end{anexample}
