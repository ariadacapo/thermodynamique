...
\index{eau!description qualitative des propriétés|(}
	...
	\index{température!énergie et}\index{énergie!température et}\index{enthalpie!température et}
...
\index{abaques de vapeur|(}\index{liquides-vapeurs!propriétés tabulées (abaques)|(}
	...
	\label{ch_points_saturation}\index{saturation!points de}\index{liquide!saturé}\index{vapeur!saturée}\index{saturé(e)!vapeur}
...
%exo_ à quelle pression faut-il porter l’eau pour qu’elle reste liquide à~150°C ?
	...
	\index{eau!description quantitative des propriétés|)}\index{abaques de vapeur|)}\index{liquides-vapeurs!propriétés tabulées (abaques)|)}
...
pharaonique. Pour s’en affranchir, il faudra monter en température : ce sera pour le \textsc{xx}\ieme siècle.
	...
	\subsection{Évolutions adiabatiques réversibles}
		...
		Comment quantifier ce $\Delta u$ ou ce $\Delta h$ ? Prenons l’exemple d’une détente adiabatique, en partant de~\SI{40}{\bar} et~\SI{500}{\degreeCelsius}. On tente d’extraire le maximum de travail de la vapeur avant de la rejeter à pression atmosphérique (\SI{1}{\bar}).\index{travail!transferts iréversibles}
