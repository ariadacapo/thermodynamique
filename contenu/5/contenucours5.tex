...
La vapeur n’est ici qu’un moyen de transporter le calorique ; elle remplit le même office que dans le chauffage des bains par la vapeur, à l’exception que dans le cas où nous sommes son mouvement est rendu utile.
	...
	\index{liquéfaction}\index{ebullition@ébullition}\index{phase, changements de}\index{changement de phase}
		...
		\begin{comment}
			From https://www.wolframalpha.com/input/?i=water+molecule : water molar mass 18 g/mol
			Et 6,022E23 molécules/mol
				Donc une densité moléculaire de 3,3456E25 molécules/kg
			From http://apollo.lsc.vsc.edu/~wintelsw/MET1010LOL/chapter07/ & http://www.geog.ucsb.edu/~joel/g110_w08/lecture_notes/precip_processes/precip_processes.html : radius of small cloud drop (visible suspension) 10 micrometers = 10E-6 m ; nucleus 0.1 micrometers = 1E-7 m ; typical raindrop r=1000 micrometers = 1mm.
				soit volume goutte nuage :						1.33E-15 m3 donc m = 1.33E-12 kg soit 4.46E13 molécules 		1
				et volume d’une petite goutte r=0,1mm :	1.33E-12 m3 donc m = 1.33E-09 kg soit 4.46E16 molécules
		\end{comment}
...
\index{eau!description qualitative des propriétés|(}
	...
	\label{ch_lv_diagramme_tv}\index{température-volume, diagramme}\index{diagramme!température-volume}
		...
		\onlyframabook{\begin{figure}[h!]}%handmade
			...
			\supercaption{Réchauffement d’une quantité fixe d’eau à pression constante.\\
				A : liquide comprimé ; B : mélange liquide-vapeur ; C : vapeur saturée ; D : vapeur sèche.}{schéma \cczero \oc}
		...
		\onlyframabook{\begin{figure}[h!]}%handmade
			...
			\end{center}
			\supercaption{Vocabulaire : état de l’eau pendant une évolution à pression constante.}{schéma \cczero \oc}
			\label{fig_T-v_construction_1}
		\end{figure}

		Au départ, lorsque l’eau est liquide, la température augmente linéairement avec le volume, avec un fort gradient. On parle alors de \vocab[liquide!comprimé]{liquide comprimé}\footnote{Liquide comprimé ou \vocab[]{sous-refroidi}.}.\index{comprimé, liquide} % en anglais : \vocabe{unsaturated}, \vocabe{compressed} or \vocabe{subcooled liquid})
		...
		\begin{figure}
			...
			\end{center}
			\supercaption{Propriétés de l’eau tracées sur un diagramme température-volume, lorsque l’on effectue l’expérience décrite en \cref{fig_changement_phase} à différentes pressions. On observe que plus la pression est grande, plus la plage d’ébullition est petite.}{schéma \cczero \oc}
	...
	\label{ch_lv_bain_marie}\index{pression-volume, diagramme}\index{diagramme!pression-volume}
		...
		\begin{figure}
			...
			\end{center}
			\supercaption{Propriétés de l’eau tracées sur un diagramme pression-volume, lorsque l’on maintient la température constante en faisant varier le volume.}{schéma \cczero \oc}
		...
		\begin{figure}
			...
			\end{center}
			\supercaption{Diagramme pression-volume de l’eau, représenté avec une évolution à température constante (isotherme). La courbe de saturation est représentée en bleu.}{schéma \cczero \oc}
			\label{fig_p-v_eau}
		\end{figure}
	...
	\subsection{Pièges pour l’étudiant/e}
		\index{liquides-vapeurs!energie@énergie interne}\index{energie@énergie!interne d’un liquide-vapeur}\index{liquides-vapeurs!enthalpie}\index{enthalpie!d’un liquide-vapeur}\index{température!enthalpie et}\index{enthalpie!température et}\index{température!energie@énergie interne et}\index{energie@énergie!interne, température et}
...
%exo_ à quelle pression faut-il porter l’eau pour qu’elle reste liquide à~150°C ?
	...
	\label{ch_melange_liquide_vapeur}\index{mélange liquide-vapeur}
		...
		\begin{figure}
			...
			\end{center}
			\supercaption{Le titre de la vapeur représenté par la position du point sur un diagramme $T-v$.}{schéma \cczero \oc}
		...
		\begin{description}
			...
			\item[L’enthalpie $h$]{d’un mélange liquide-vapeur est égale à la somme de l’enthalpie du liquide et de celle du gaz. On a ainsi, comme illustré en \cref{fig_titre_h} :\index{mélange liquide-vapeur!enthalpie d’un}
				\begin{IEEEeqnarray}{rCl}
					h_x 	& = & (1-x) h_L + x \ h_V 		\nonumber \\
						& = & h_L + x (h_V - h_L)				\nonumber \\
					h_x 	& = & h_L + x \ h_{LV}
				...
				\begin{figure}
					...
					\end{center}
					\supercaption{Enthalpie $h_x$ d’un mélange en fonction des enthalpies à l’état saturé et de vaporisation.}{schéma \cczero \oc}
					\label{fig_titre_h}
				\end{figure}
			} %end item

			\index{energie@énergie!interne d’un liquide-vapeur}\index{mélange liquide-vapeur!energie@énergie interne d’un}
			...
			\item[Le volume spécifique]{d’un mélange liquide-vapeur, enfin, se quantifie encore plus simplement. Le volume total du mélange est égal au volume du gaz plus celui du liquide, et donc :
				...
				Il faut bien noter que l’échelle en abscisse est logarithmique : $v_L$ est en général plusieurs centaines de fois plus petit que $v_V$, et l’approximation n’est pas mise en valeur graphiquement.}{schéma \cczero \oc}
					\label{fig_titre_v}
				\end{figure}
			} % end item
...
\index{evolutions@évolutions!d’un liquide-vapeur|(}
	...
	Il est possible de chauffer ou refroidir un liquide/vapeur en maintenant sa pression constante (\cref{fig_lv_pression_constante}). Une évolution à pression constante est dite \vocab[evolutions@évolutions!isobares]{isobare}\index{isobares, évolutions}.
		...
		\begin{figure}
			...
			\end{center}
			\supercaption{Évolution à pression constante (isobare) d’un liquide/vapeur. En système fermé (à gauche), le piston exerce une force constante tout au long de l’évolution. En système ouvert (à droite), aucun travail n’est effectué.}{schéma \ccbysa \olivier}
		...
		\begin{figure}
			...
			\end{center}
			\supercaption{Réchauffement à pression constante d’un liquide/vapeur, représenté sur un diagramme pression-volume.}{schéma \cczero \oc}
		...
		\begin{anexample}\label{exemple_lv_isobare}
...
pharaonique. Pour s’en affranchir, il faudra monter en température : ce sera pour le \textsc{xx}\ieme siècle.
	...
	\label{ch_lv_isochores}
		...
		Il est possible de chauffer ou refroidir un liquide/vapeur en maintenant son volume constant (\cref{fig_lv_isochore}). Une évolution à volume constant est dite \vocab[evolutions@évolutions!isochores]{isochore}\index{isochores, évolutions}.

		\begin{figure}
			\begin{center}
				\includegraphics[width=8cm]{images/lv_isochore.png}
			\end{center}
			\supercaption{Évolution à volume constant (isochore) d’un liquide/vapeur. En système fermé (à gauche), le volume est bloqué et aucun travail n’est effectué. En système ouvert (à droite), on doit comprimer le fluide pendant qu’on le chauffe et le détendre pendant qu’on le refroidit, pour pouvoir maintenir le volume spécifique constant.}{schéma \ccbysa \olivier}
		...
		\begin{figure}
			...
			\end{center}
			\supercaption{Réchauffement à volume constant d’un liquide/vapeur, représenté sur un diagramme pression-volume.}{schéma \cczero \oc}
	...
	\label{ch_lv_isothermes}
		...
		Il est possible de chauffer ou refroidir un liquide/vapeur en maintenant sa température constante (\cref{fig_lv_isotherme}). Une évolution à température constante est dite \vocab[evolutions@évolutions!isothermes]{isotherme}\index{isothermes, évolutions}.

		\begin{figure}
			\begin{center}
				\includegraphics[width=11cm]{images/lv_isotherme.png}
			\end{center}
			\supercaption{Évolution à température constante (isotherme) d’un liquide/vapeur. En système fermé (à gauche), on laisse travailler le gaz sur un piston pendant qu’on le chauffe, et à l’inverse, on lui fournit du travail lorsqu’on le refroidit. En système ouvert (à droite), les mêmes manipulations sont effectuées en flux continu.}{schéma \ccbysa \olivier}
		...
		\begin{figure}
			...
			\end{center}
			\supercaption{Détente (réchauffement) à température constante d’un liquide/vapeur, représenté sur un diagramme pression-volume.}{schéma \cczero \oc}
	...
	\subsection{Évolutions adiabatiques réversibles}
		...
		Une évolution adiabatique \emph{réversible}\index{evolutions@évolutions!adiabatiques réversibles}\index{adiabatiques!réversibles, évolutions} est effectuée infiniment lentement. Un piston dans un cylindre devra pour cela être déplacé infiniment lentement, et une turbine en flux continu devra pour cela être infiniment longue. Les évolutions adiabatiques servent de référence, d’objectif théorique, pour quantifier les performances des turbines réelles, que nous étudierons au \coursneufshort.
		...
		\begin{figure}
			...
			\end{center}
			\supercaption{Évolution adiabatique réversible (isentropique) d’un liquide/vapeur. En système fermé (à gauche) comme en système ouvert (à droite), le fluide est parfaitement isolé, de sorte qu’il n’y ait aucun transfert de chaleur, même si sa température varie.}{schéma \ccbysa \olivier}
		...
		\begin{figure}
			...
			\end{center}
			\supercaption{Détente adiabatique réversible d’un liquide/vapeur, représentée sur un diagramme pression-volume.}{schéma \cczero \oc}
		...
		Plus tard, dans le \courshuit, nous verrons qu’une évolution adiabatique réversible se fait à \vocab{entropie} constante (c’est pour cela que nous appellerons ces transformations \vocab[evolutions@évolutions!isentropiques]{isentropiques}\index{isentropique(s)!évolutions}), et nous nous servirons de cet outil phénoménal pour répondre à ces questions.
