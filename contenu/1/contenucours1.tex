\section{Notion d’énergie}
	...
	\label{ch_energie}
	\index{energie@énergie!concept|(textbf}
	...
	\subsection{Le premier principe}
		...
		Le \vocab[principes de la thermodynamique!premier|textbf]{premier principe de la thermodynamique} affirme simplement :
		...
		Nous exprimerons quantitativement le premier principe de deux façons différentes, l’une pour un système fermé (au \coursdeuxshort, \cref{eq_premier_principe_sf_min}) et l’autre pour un système ouvert (au \courstroisshort, équation~\ref{eq_petite_sfee_deltas_h}).
	
	
	\subsection{Formes d’énergie}
	\index{energie@énergie!formes}
		...
		L’\vocab[energie@énergie!cinétique]{énergie cinétique} est possédée par un corps du fait de sa vitesse (\textit{cf.} le paragraphe \S\ref{ch_energie_mecanique} plus bas). C’est la forme d’énergie la plus facile à identifier. Elle a longtemps été nommée \textit{vis viva} («~force vive~»).
		
		L’\vocab[energie@énergie!potentielle]{énergie potentielle} est stockée avec l’interaction entre deux objets liés par une force conservative\footnote{Une force est dite \vocab[force!conservative]{conservative} lorsqu’elle reste la même dans un sens comme dans un autre. Par exemple, le poids est conservatif (il est le même que l’on monte ou que l’on descende) mais le frottement ne l’est pas (il s’oppose toujours au mouvement).}. À l’échelle macroscopique, la forme la plus palpable est l’énergie potentielle d’altitude, issue du travail fourni à une masse contre son poids (c’est ce travail qui rend plus fatigante la montée d’escaliers que leur descente, par exemple). En écrasant un ressort, on y stocke de l’énergie potentielle de compression, que l’on récupère en le détendant.

		L’\vocab[energie@énergie!chimique]{énergie chimique} est une combinaison d’énergie potentielle et d’énergie cinétique \emph{entre atomes}. Le métabolisme humain, ainsi que la combustion des hydrocarbures avec l’oxygène atmosphérique utilisée dans presque tous nos véhicules, sont tous deux fondés sur des transferts énergétiques chimiques.
		
		\index{energie@énergie!masse}\index{energie@énergie!rayonnante}
		...
		\index{energie@énergie!interne}\index{chaleur!concept}\index{travail}\index{macroscopique, échelle}
	...
	\index{Watt!unité}
		...
		\begin{figure}
			...
			\end{center}
			\supercaption{Une remorque, de puissance nulle ($\dot Q = \SI{0}{\watt}$) mais capable de restituer beaucoup d’énergie. La combustion de~\SI{20}{\tonne} de kérosène dégage environ $Q = \SI{800}{\giga\joule}$ sous forme de chaleur ; \\
				Un turboréacteur à soufflante \textit{\we{Trent 900}}, de très grande puissance (capable de fournir $\dot W = \SI{14}{\mega\watt}$ à un avion de ligne) mais dépourvu d’énergie (\SI{0}{\joule}).}{Photo turboréacteur dérivée d’\wcfile{Airbus_Lagardère_-_Trent_900_engine_MSN100_(2).JPG}{une photo} \cczero par \wcu{Dr Brains}\\ Photo remorque dérivée d’\wcfile{FedEx_Express_fuel_tank_trailer_at_Memphis_International_Arirport.jpg}{une photo} \ccby par Thomas R Machnitzki}
			\label{fig_energie_puissance}%
			\begin{comment}
				Remorque : 20t de kérosène @ 40 MJ/kg = 800E9 J
				A380 : W=500t, finesse f=20, n=4 moteurs, V=800 km/h = 222 m/s. Alors P = V T = V W/(n f) = 13,6E6 Watt
			\end{comment}
		\end{figure}
		
		\index{cheval-vapeur (unité)}
	...
	\index{massique!energie@énergie}\index{massique!puissance}\index{energie@énergie!spécifique}\index{puissance!spécifique}\index{spécifique!energie@énergie}\index{spécifique!puissance}
		...
		À cet effet, nous utilisons des grandeurs dites \vocab[spécifique!grandeur, notion de|textbf]{spécifiques} (dites parfois \mbox{\vocab[massique!grandeur, notion de]{massiques}}) ; et nous les notons en minuscules.

		\begin{description}
		
			\index{energie@énergie!spécifique}
			\item[L’énergie spécifique] (parfois appelée énergie massique), se mesure en \si{joules} \si{par} \si{kilogramme}~(\si{\joule\per\kilogram}) :
				...
				\begin{equationterms}
					...
					\item et \tab $m$	\tab la masse du système que l’on étudie (\si{\kilogram}).\index{masse!quantité de}
				\end{equationterms}
		...
		\index{puissance!spécifique}
		\item[La puissance spécifique] (parfois également appelée puissance massique), a les mêmes unités que l’énergie spécifique : on divise des \si{watts} (\si{joules} \si{par} \si{seconde}) par un débit de masse (\si{kilos} \si{par} \si{seconde}).
			\begin{equation}
			...
			\begin{equationterms}
				...
				\item et \tab $\dot{m}$ \tab le débit de masse traversant le système (\si{\kilogram\per\second}).\index{masse!débit de}
			\end{equationterms}
...
\label{ch_energie_mecanique}
	...
	L’étudiant/e n’aura aucun mal à quantifier l’\vocab[energie@énergie!cinétique]{énergie cinétique} :
	\begin{equation}
	E_{c} = \frac{1}{2} \ m \ C^2
	...
	On définit bien sûr de façon correspondante l’\vocab[spécifique!energie@énergie cinétique]{énergie cinétique spécifique} :
	\begin{equation}
	...
	L’expression de l’\vocab[energie@énergie!potentielle]{énergie potentielle d’altitude} ne devrait pas non plus faire sourciller l’étudiant/e :
	...
	Énergie cinétique et potentielle d’altitude sont souvent rassemblées en un seul terme, nommé \vocab[energie@énergie!mécanique]{énergie mécanique} :
	\begin{equation}
...
\label{ch_travail_fdl}
	...
	Le \vocab[travail|textbf]{travail} est un transfert d’énergie. Un objet fournit un travail (et perd ainsi de l’énergie) lorsqu’il exerce une force le long d’un déplacement. En mécanique, ce travail est quantifié à l’aide de vecteurs :
	\begin{equation}
	...
	Sur un graphique représentant la force en fonction de la distance, ce travail $W_\fromatob$ est représenté par la surface sous la courbe de A à B (\cref{fig_force-déplacement-aire}). La géométrie de la courbe, c’est-à-dire la relation $F_{(l)}$ entre $F$ et $l$ au fur et à mesure de l’évolution, déterminera la quantité $W_\fromatob$.
	
	\onlyamphibook{\begin{figure}}%handmade
	\onlyframabook{\begin{figure}[h!]}%handmade
	\begin{center}
		\includegraphics[width=8cm]{images/fl.png}
	\end{center}
	\supercaption{Sur un diagramme force-distance, le travail fourni par un objet peut être visualisé avec l’aire sous la courbe. Dans le cas montré ici, l’objet voit sa longueur $l$ augmenter, et le travail sera négatif (fourni par l’objet).}{schéma \cczero \oc}
	\label{fig_force-déplacement-aire}
	\end{figure}

		\clearfloats
		\begin{anexample}\index{ressorts!compression de}\index{travail!d’un ressort}
...
% L column in tabulary solves this but tabulary precludes using arydshln
		...
		\end{spacing}
		\end{scriptsize}
		\caption{Quelques exemples de températures. Les valeurs les moins précises, dénotées par un astérisque, sont converties approximativement.}\vspace{0.5em}%handmade
	...
	\index{chaleur!concept|textbf}
		...
		\thermoquoteend{Rudolf Clausius, 1850~\mbox{\cite{clausius1850, clausius1850en, clausius1868fr1}}\onlyframabook{\vspace{-1cm}}}{}%handmade

		Lorsque l’on met deux corps de températures différentes en contact, leurs températures ont tendance à s’égaliser au cours d’un transfert spontané d’énergie. Nous appelons cette forme d’énergie \vocab[chaleur!concept|textbf]{chaleur}.\index{chaleur!nature de la}
		...
		\onlyframabook{\end{itemize}\par\vspace{-1em}\begin{itemize}}%handmade, hack tout pourri pour faire tenir la citation sur le côté
		...
		De même que l’on note $Q$ la chaleur (\si{\joule}), on note $q$ la \vocab[chaleur!spécifique]{chaleur spécifique}\index{chaleur!massique} (\si{\joule\per\kilogram}).
		
		\index{calorique (fluide, hist.)}\index{chaleur!conservation (ou non)}\index{microscopique, échelle}
	...
	\index{capacité!thermique|textbf}
		...
		\end{equation}
		\begin{equationterms}
			\item où \tab $c$			\tab\tab est la \vocab[capacité!thermique massique]{capacité thermique massique} du corps considéré (\si{\joule\per\kilogram\per\kelvin}),
...
\section{Le chaud et le froid}
	...
	Nous terminons ce chapitre en re-visitant quelques termes de langage courant tels que nous les comprenons en thermodynamique :
		...
		\end{figure}
			...
			\onlyamphibook{\end{description}%handmade -- cette citation n’apparaît que dans l’amphibook (dans le framabook elle fait terminer le chapitre sur un paragraphe de 3 lignes). Hack un peu pourri pour faire tenir la citation sur le côté.
				...
				\thermoquoteend{William Thomson\\{\tiny(non encore couronné \textit{Baron Kelvin}…)}\index{Kelvin!William Thomson}}{1848~\cite{kelvin1848}\vspace{2em}} %handmade vspace
				\vspace{-0.7em}~\vspace{-1.8em}
			\begin{description}}%fin onlyamphibook
			\item \textit{Le thermomètre} --- Nous laissons à l’étudiant/e le loisir d’explorer le fonctionnement des \vocab[thermomètre]{thermomètres} : comment peut-on \emph{savoir} dans l’absolu qu’une température est haute ou basse ?\\
