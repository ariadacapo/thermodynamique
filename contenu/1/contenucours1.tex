\section{Notion d’énergie}
	...
	\subsection{Le premier principe}
		...
		Le \vocab[principes de la thermodynamique!premier]{premier principe de la thermodynamique} affirme simplement :
	...
	\subsection{Formes d’énergie}
		...
		L’\vocab[énergie!cinétique]{énergie cinétique} est possédée par un corps du fait de sa vitesse (\textit{cf.} \cref{ch_energie_mecanique} plus bas). C’est la forme d’énergie la plus facile à identifier. Elle a longtemps été nommée \textit{vis viva} («~force vive~»).
		
		L’\vocab[énergie!potentielle]{énergie potentielle} est stockée avec l’interaction entre deux objets liés par une force conservative\footnote{Une force est dite \vocab[force!conservative]{conservative} lorsqu’elle reste la même dans un sens comme dans un autre. Par exemple, le poids est conservatif (il est le même que l’on monte ou que l’on descende) mais le frottement ne l’est pas (il s’oppose toujours au mouvement).}. À l’échelle macroscopique, la forme la plus palpable est l’énergie potentielle d’altitude, issue du travail fourni à une masse contre son poids (c’est ce travail qui rend plus fatigante la montée d’escaliers que leur descente, par exemple). En écrasant un ressort, on y stocke de l’énergie potentielle de compression, que l’on récupère en le détendant.

		L’\vocab[énergie!chimique]{énergie chimique} est une combinaison d’énergie potentielle et d’énergie cinétique \emph{entre atomes}. Le métabolisme humain, ainsi que la combustion des hydrocarbures avec l’oxygène atmosphérique utilisée dans presque tous nos véhicules, sont tous deux fondés sur des transferts énergétiques chimiques.
		
		\index{énergie!masse}\index{énergie!rayonnante}
		...
		En thermodynamique, nous allons nous concentrer sur trois formes d’énergie, identifiables à l’échelle macroscopique :\index{macroscopique, échelle}
		
		\begin{description}\index{énergie!interne}\index{chaleur}\index{travail}
	...
	\subsection{La puissance}
		...
		D’autres unités sont souvent utilisées, comme le cheval-vapeur\index{cheval-vapeur (unité)}%
	...
	\index{énergie!massique}\index{puissance!massique}\index{massique, énergie}\index{massique, puissance}\index{énergie!spécifique}\index{puissance!spécifique}\index{spécifique, énergie}\index{spécifique, puissance}
		...
		À cet effet, nous utilisons des grandeurs dites \vocab[spécifique!grandeur, notion de]{spécifiques} (dites parfois \vocab[massique!grandeur, notion de]{massiques}) ; et nous les noterons en minuscules.

		\begin{description}
		
			\item[L’énergie spécifique]\index{énergie!spécifique} (parfois appelée énergie massique), se mesure en \si{joules} \si{par} \si{kilogramme}~(\si{\joule\per\kilogram}) :
				\begin{equation}
					e \equiv \frac{E}{m}
		...
		\item[La puissance spécifique]\index{puissance!spécifique} (parfois également appelée puissance massique), a les mê\-mes unités : on divise des \si{watts} (\si{joules} \si{par} \si{seconde}) par un débit de masse (\si{kilos} \si{par} \si{seconde}).
			\begin{equation}
...
\label{ch_energie_mecanique}
	...
	L’étudiant/e n’aura aucun mal à quantifier l’\vocab[énergie!cinétique]{énergie cinétique} :
	\begin{equation}
	E_{c} = \frac{1}{2} \ m \ C^2
	...
	On définit bien sûr de façon correspondante l’\vocab[spécifique!énergie cinétique]{énergie cinétique spécifique} :
	\begin{equation}
	...
	L’expression de l’\vocab[énergie!potentielle]{énergie potentielle d’altitude} ne devrait pas non plus faire sourciller l’étudiant/e :
	...
	Énergie cinétique et potentielle d’altitude sont souvent rassemblées en un seul terme, nommé \vocab[énergie!mécanique]{énergie mécanique} :
	\begin{equation}
...
\label{ch_travail_fdl}
	...
	\begin{equationterms}		
		...
		\item et \tab $\vec l$ 	\tab le vecteur représentant le déplacement effectué (de norme $l$ en \si{\metre}).\index{déplacement}
	\end{equationterms}
	...
	Sur un graphique représentant la force en fonction de la distance\index{diagramme!force-distance}\index{force-distance, diagramme}, ce travail $W_\fromatob$ est représenté par la surface sous la courbe de A à B (\cref{fig_force-déplacement-aire}). La forme de la courbe, c’est-à-dire la relation $F_{(l)}$ entre $F$ et $l$ au fur et à mesure de l’évolution, déterminera la quantité $W_\fromatob$.
...
\section{La chaleur}
	...
	\label{ch_définition_température_cours1}
		...
		. Il suffit de soustraire 273,15 unités à une température absolue (en \si{kelvins}) pour lire une température en \si{degrés} \si{Celsius} :\index{Celsius!degrés (échelle)}\index{Kelvins (unité)}
		\begin{equation}
...
% L column in tabulary solves this but tabulary precludes using arydshln
	...
	\index{thermique, capacité}\index{calorifique, capacité}
		...
		\begin{itemize}
			...
			\item La capacité calorifique massique des liquides et vapeurs devient infinie~(!) pendant l’ébullition\index{ébullition}, qui a lieu sur une plage particulière de propriétés. Hors de cette plage, la capacité redevient finie mais elle varie avec la température. Nous décrirons ces comportements au \courscinq.
...
\section{Le chaud et le froid}
	...
	Nous terminons ce chapitre en re-visitant quelques termes de langage courant tels que nous les comprenons en thermodynamique :
		...
		\begin{description}
			...
			\item \textit{Le chaud}\index{chaud} --- Pour nous, «~chaud~» n’est pas une propriété des corps : plutôt que «~cet objet est chaud~» nous dirons qu’il est à haute température. Plutôt que «~cet objet s’échauffe/se refroidit~» nous dirons que sa température augmente ou diminue.\\
			...
			\item \textit{Chauffer}\index{chauffer} --- Pour nous, «~chauffer~» c’est fournir de la chaleur. On peut «~chauffer~» un corps pendant que sa température chute. On peut également faire augmenter la température d’un corps sans lui apporter de chaleur (\cref{fig_chauffer_refroidir}).
