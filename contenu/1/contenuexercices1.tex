...
\label{exo_refroidissement_centrale_vapeur}
	...
	Le circuit suivi par l’eau dans les centrales à vapeur peut être représenté de façon simplifiée de la façon suivante (\cref{fig_cycle_rankine_chap_un}) :
			...
			Ce circuit, nommé cycle de Rankine, est étudié de façon plus approfondie au \coursneuf (\cref{ch_cycle_de_rankine} p.\pageref{ch_cycle_de_rankine}).}{schéma \ccbysa \olivier}
...
\wherefrom{[dér. DS n°1 2012 7pts]}
	...
	\end{center}
	\supercaption{Expérience réalisée avec un ressort puissant. Le piston écrase le ressort de $1$ à $2$, puis le ressort repousse le piston de $3$ à $4$. Au retour, la force exercée par le ressort est plus grande.}{schéma \ccbysa \olivier}
...
\wherefrom{[DS n°1 2012, 5pts]}
	...
	\begin{figure}
		...
		\end{center}
		\supercaption{Schéma de principe d’un cric hydraulique.}{schéma \cczero \oc}
		\label{fig_cric}
	\end{figure}
...
\label{exo_turbine_eau_puissances_spe}
	...
	\begin{figure}
		...
		\end{center}
		\supercaption{Schéma de principe d’une turbine à eau. L’eau pénètre en haut à gauche, fait tourner les pales de la turbine, est réchauffée par frottement, et ressort en bas à droite de l’installation.}{schéma \ccbysa \olivier}
...
\label{exo_chaudiere_simple}\index{chaudière}\index{chaudière!efficacité}
	...
	\onlyamphibook{\begin{figure}}%handmade
		\begin{center}
			\onlyamphibook{\includegraphics[width=0.7\textwidth]{images/chaudiere_eau.png}}
			\onlyframabook{\includegraphics[width=0.9\columnwidth]{images/chaudiere_eau.png}}%handmade
		\end{center}
		\supercaption{Schéma de principe d’une chaudière utilisée pour le chauffage d’un bâtiment. L’eau ($\C \to D$) y pénètre par la droite, et y est réchauffée par l’air ($\A \to \B$) mélangé au kérosène.}{schéma \ccbysa \olivier}
...
\label{exo_turbomoteur_puissances_spe}
	...
	Un hélicoptère est muni de deux turbomoteurs, c’est à dire de turbomachines dont le but est de faire tourner un arbre sortant du moteur (\cref{fig_s76b}). Nous pouvons évaluer plusieurs caractéristiques de ces moteurs sans connaître précisément leur fonctionnement interne.\onlyframabook{\pagebreak}%handmade pour permettre de tout mettre sur la même page
...
\begin{description}
	\item[\ref{exo_velo_roue_libre}]
				...
				\tab 2) $\dot Q_{\text{frottements}} = m \ g \ \left|\dot z\right| = \SI{368}{\watt}$.
	...
	\item [\ref{exo_ressorts_moteur}]
				...
				\tab 2) $W_{1 \to 2} = \SI{+3,196}{\kilo\joule}$
				\tab 3) $F_{(l) 3 \to 4} = \num{35,2e3} - \num{40e3} l$
				\tab 4) $W_{3 \to 4} = \SI{-4,862}{\kilo\joule}$
				\tab 5) $W_{\text{cycle}} = \SI{1,666}{\kilo\joule}$
				\tab 6) $f = \SI{11,04}{\per\second}$ (11 fois par seconde)
	\item [\ref{exo_bain}]
	...
	\item [\ref{exo_turbomoteur_puissances_spe}]
				...
				\tab 2) $q_{\text{chambre}} + w_{\text{arbre}} + q_{\text{refroidissement atmosphérique}} = 0$ ou encore $q_{\text{chambre}} + w_{\text{arbre}} - q_{\text{rejet}} = 0$ ; ainsi on a $w_{\text{arbre}} = - q_{\text{chambre}} + q_{\text{rejet}} = \SI{-190,1}{\kilo\joule\per\kilogram}$ (ces puissances ne dépendent pas du débit de masse, et donc pas du nombre de moteurs considérés).
				\tab 3) $\dot m_{\text{air}} = \SI{6,95}{\kilogram\per\second}$
				\tab 4) $\dot Q_{\text{chambres}} = \SI{3,836}{\mega\watt}$
				\tab 5) $\dot m_{\text{kérosène}} = \frac{\dot Q_{\text{chambres}}}{q_{\text{kérosène}}} = \SI{300,2}{\kilogram\per\hour}$ (valeur réaliste).
\end{description}
