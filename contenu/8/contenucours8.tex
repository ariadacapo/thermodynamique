...
\section{Le concept de l’entropie}
	...
	\index{evolutions@évolutions!sens des}\index{sens!des évolutions}
	
		Dans le vocabulaire de la thermodynamique, le concept d’une «~évolution à sens unique~» est bien sûr nommé \vocab[irréversibilité!concept]{irréversibilité}. Nous avions déjà abordé l'irréversibilité en section \S\ref{ch_évolutions_irr_sf}, où nous avions déterminé qu’elle avait deux causes principales :
			\begin{itemize}
				\item La transformation d’un travail en chaleur, par frottement et turbulence ;\index{travail!transformation en chaleur}\index{chaleur!transformation de travail en}
				\item La transmission d’une quantité de chaleur entre deux corps de températures différentes.\index{chaleur!transferts irréversibles}
			\end{itemize}
		...
		\index{chaleur!lors des irréversibilités}\index{irréversibilité!quantification}
		\thermoquotebegin{O}
		...
		\thermoquoteend{Rudolf Clausius, 1856~\cite{clausius1856, clausius1867en,clausius1868fr3}\index{Clausius!Rudolf}}{}
...
\section{Définition}
	...
	\label{ch_entropie_propriete}
		...
		Commençons par admettre le fait que l’entropie est une \vocab[propriété physique]{propriété} physique\footnote{On dit aussi \vocab[variable thermodynamique]{variable physique} ou \vocab{variable thermodynamique}.}, c’est-à-dire quelque chose qui caractérise l’état d’un système. Dit autrement : si l’on considère une portion de l’univers à un moment donné (un système), nous trouvons que ce système a une masse, un volume, une température : ce sont ses propriétés, des descriptions absolues de son état. L’entropie est une de ces propriétés.
	...
	\label{ch_entropie_definition}\index{entropie!définition|textbf}
		...
		L’unité de l’entropie, $S$, est le \si{\joule\per\kelvin} (\si{joule} par \si{kelvin}) ; et de façon correspondante, on définit l’\vocab[entropie!spécifique]{entropie spécifique}\index{spécifique!entropie} $s$ :
		\begin{equation}
			s \equiv \frac{S}{m}
		\end{equation}
		\begin{equationterms}
			...
			Une fois que le refroidissement est effectué, on laisse l’air se détendre brutalement : la détente est irréversible. Pendant celle-ci, on ne fournit que \SI{1000}{\joule} sous forme de chaleur et on ne récupère que \SI{1000}{\joule} sous forme de travail. À la fin de la détente, le gaz est dans le même état (mêmes température, pression, et énergie interne) qu’au tout début de l’expérience. Quelle est la variation d’entropie ?
				...
				Heureusement, nous savons que le gaz retourne de B à son état initial A ; or de A à B l’évolution était réversible. En effectuant l’évolution exactement inverse, nous inverserions tous les transferts de chaleur et de travail. Nous aurions alors, le long de cette évolution imaginaire de B vers A : $\Delta S = S_A - S_B
					...
					= \SI{+6,6}{\joule\per\kelvin}$.
						...
						\begin{remark} Ici nous voyons que la chaleur transférée en réalité n’a pas d’importance. C’est la chaleur «~qu’il aurait fallu transférer~» qui nous intéresse.\end{remark}\index{chaleur!lors des irréversibilités}\index{chaleur!transferts réversibles}
	...
	\subsection{Remarques}
		...
		Ajoutons trois remarques avant de poursuivre.
			...
			\begin{enumerate}
				...
				\index{entropie!mesure (impossibilité)}
...
\label{ch_variations_entropie}
	...
	\index{entropie!analogie avec le volume}\index{température!entropie et}
		...
		Alors, nous allons pouvoir représenter les évolutions sur un \vocab[diagramme!température-entropie]{diagramme température-entropie}\index{température-entropie, diagramme}. L’aire sous la courbe d’une évolution représentera la chaleur transmise si l’évolution est réversible, mais pas si elle est irréversible, comme montré en \cref{fig_ts_reversible_irreversible}.\index{chaleur!lors des irréversibilités}
	...
	\index{diagramme!pression-volume}\index{diagramme!température-entropie}\index{pression-volume, diagramme}\index{température-entropie, diagramme}
		...
		\clearfloats\index{cycle!thermodynamique}
		...
		\onlyamphibook{\clearfloats}\index{Carnot!cycle de}\index{cycle!de Carnot}\index{diagramme!pression-volume}\index{diagramme!température-entropie}\index{pression-volume, diagramme}\index{température-entropie, diagramme}
	...
	\label{ch_delta_s_gaz_parfaits}\index{entropie!d’un gaz parfait, variations d’}\index{gaz parfait!variations d’entropie d’un}
		...
		Or, si nous utilisons un gaz parfait, nous avons $u = c_v T$ (\ref{eq_principe_de_joule})	et $\frac{p}{T} = \frac{R}{v}$ (\ref{eq_pv=RT}), ainsi :
			...
			\thermoquoteend{Rudolf Clausius, 1865~\cite{clausius1865,clausius1867en,clausius1868fr2}\index{Clausius!Rudolf}}{}
		...
		\onlyframabook{\begin{figure}[h!]}
			\begin{center}
				\includegraphics[width=8cm]{images/ts_gp_isobares_isochores.png}
			\end{center}
	...
	\index{entropie!d’un liquide-vapeur, variations d’}\index{liquides-vapeurs!variations d’entropie d’un}\index{mélange liquide-vapeur!entropie d’un}\index{abaques de vapeur}\index{vapeur!abaques de}\index{liquides-vapeurs!propriétés tabulées (abaques)}
		...
		\begin{figure}
			...
			\end{center}
			\supercaption{Diagramme température-entropie d’un liquide/vapeur. Cette figure est très similaire à la \cref{fig_t-v_eau} p.\pageref{fig_t-v_eau}.}{schéma \cczero \oc}
			\label{fig_ts_lv}
		\end{figure}
		...
		\index{titre}\index{vapeur!titre de}
...
\index{evolutions@évolutions!sens des}\index{sens!des évolutions}\index{irréversibilité!quantification|(}
	...
	\index{irréversibilité!lors des transferts de chaleur}\index{chaleur!transferts irréversibles}
		...
		\thermoquoteend{Rudolf Clausius, 1854~\cite{clausius1854,clausius1867en,clausius1868fr4}\index{Clausius!Rudolf}}{}
		...
		\onlyframabook{\clearfloats}%handmade
		...
		\onlyamphibook{\dontbreakpage}%handmade

	\subsection{Irréversibilités lors des compressions et détentes adiabatiques}
	\index{adiabatiques!irréversibles, évolutions}\index{chaleur!transformation de travail en}\index{travail!transformation en chaleur}\index{chaleur!lors des irréversibilités}\index{irréversibilité!lors des transferts de travail}\index{travail!transferts irréversibles}
		...
		\onlyframabook{\begin{figure}[h!]}
			\begin{center}
				\includegraphics[width=0.4\textwidth]{images/ts_gp_detente.png}
				\includegraphics[width=0.4\textwidth]{images/ts_gp_compression.png}
				\onlyframabook{\vspace{-0.5cm}}%handmade
			\end{center}
	...
	\index{principes de la thermodynamique!second, expression avec l’entropie}
		...
		\begin{description}	
			\index{irréversibilité!lors des transferts de chaleur}\index{chaleur!transferts irréversibles}\index{température!gradient de}
		...
		\index{energie@énergie!concept}\index{entropie!concept}
	...
	\index{evolutions@évolutions!sens des}\index{sens!des évolutions}
		...
		Il y a trois possibilités :
			\begin{itemize}
				\item Si les deux intégrales sont égales, alors la transformation réelle est \vocab[reversibilite@réversibilité]{réversible} : elle peut avoir lieu dans les deux sens.
				\item Si $\int \left(\frac{\diff q}{T}\right)_\text{chemin réel} < \int \left(\frac{\diff q}{T}\right)_\text{rév.}$, alors la transformation réelle est \vocab[réversibilité]{irréversible}. Elle ne peut avoir lieu que depuis A vers B.
				\item Si $\int \left(\frac{\diff q}{T}\right)_\text{chemin réel} > \int \left(\frac{\diff q}{T}\right)_\text{rév.}$, alors la transformation «~réelle~» décrite est \vocab[impossibilité (physique)]{impossible}. Elle ne peut avoir lieu que dans le sens inverse (B $\to$ A).
			\end{itemize}
...
\section{L’entropie, le temps, et l’univers}
	...
	\index{evolutions@évolutions!sens des}\index{sens!des évolutions}\index{sens!du temps}
		...
		\index{Clausius!Rudolf}\index{Reech, Ferdinand}\index{Rankine!William John Macquorn}
		C’est ce désir de retrouver l’ordre absolu dans lequel se succèdent les états, c’est-à-dire le sens du temps, qui a mené le physicien allemand \wf{Rudolf Clausius} jusqu’à proposer le concept d’\vocab[entropie!concept]{entropie} en 1865 dans une publication magistrale --\ \textit{Über verschiedene für die Anwendung bequeme Formen der Hauptgleichungen der mechanischen Wärmetheorie}~\cite{clausius1865,clausius1867en,clausius1868fr2}. Concluant une décennie de travail autour de la grandeur $\frac{Q}{T}$, il y formalise un concept que ses condisciples français \wf{Ferdinand Reech} et écossais \wf{William Rankine} n’avaient qu’effleuré~\cite{truesdell1980}, et y rassemble l’ensemble des connaissances contemporaines de sa discipline.
		
		\onlyamphibook{\thermoquotebegin{O}}
		\onlyframabook{\thermoquotetopbegin{O}}
			J’ai intentionnellement formé le mot \vocab{entropie} de sorte qu’il soit aussi similaire au mot \vocab[energie@énergie!concept]{énergie} que possible, parce que les sens physiques des deux grandeurs nommées par ces mots sont si étroitement liées qu’il m’a semblé qu’une cetraine similarité dans leurs désignations était désirable.
		\thermoquoteend{Rudolf Clausius, 1865~\cite{clausius1865,clausius1867en,clausius1868fr2}\index{Clausius!Rudolf}}{\onlyframabook{\vspace{-1em}}\onlyamphibook{\vspace{-1cm}}}%handmade
		\index{entropie!etymo@étymologie}
		...
		Ainsi, après un siècle d’efforts, la physique de la chaleur a rattrapé la technologie des moteurs. Nous voilà capables de décrire entièrement et de façon quantitative le comportement des corps sans devoir nous pencher sur celui de leurs constituants, tels que les molécules, atomes ou particules sub-atomiques : l’entropie était la dernière pièce manquante de ce que nous appelons aujourd’hui la \vocab[macroscopique, échelle]{thermodynamique macroscopique}.
	...
	\label{ch_entropie_boltzmann}
		...
		\index{Boltzmann!Ludwig}\index{Boltzmann!entropie selon}
		Il ne faudra que dix ans pour que la réponse soit formalisée : l’autrichien \wf{Ludwig Boltzmann} propose en 1875 une \vocab[entropie!définition microscopique]{définition microscopique}\index{microscopique, échelle}\index{entropie!selon Boltzmann} de l’entropie :
			\begin{equation}
				S \equiv k \ln \lambda
			\end{equation}
			\begin{equationterms}
...
Ainsi, avec la définition technique ci-dessus du désordre, nous pouvons comprendre la proposition. Premièrement, l’entropie mesure le désordre. Deuxièmement, l’univers va toujours de «~l’ordre~» vers le «~désordre~», ainsi l’entropie augmente toujours. L’ordre n’est pas un ordre au sens où nous aimons l’arrangement, mais au sens où le nombre de manières différentes à notre disposition pour obtenir quelque chose de toujours extérieurement identique, est relativement restreint.
		\thermoquoteend{Richard Feynman, 1963~\cite{feynman1963, feynman1963fr}\index{Feynman, Richard}}{\onlyframabook{\vspace{-3em}}}%handmade
		...
		\index{impossibilité (physique)}\index{probabilité (physique)}
		...
		\index{statistique, thermodynamique}
		Cette approche a non seulement le mérite de raccrocher notre discipline avec la théorie atomique --\ et nous parlerons dès lors de \vocab[microscopique, échelle]{thermodynamique microscopique} et \vocab[thermodynamique statistique]{statistique}\ -- mais elle va aussi ouvrir la porte de la théorie de l’information. En effet, la résolution et la précision avec lesquelles on évalue l’état d’un système affectent le nombre de configurations possibles que l’on peut lui attribuer. Voici le concept de l’\vocab[information (physique)]{information} rattaché à d’autres propriétés physiques : un résultat impressionnant pour une discipline qui ne visait qu’à explorer ce que voulait dire~«~chaud~» !
		
	\subsection{L’entropie et l’univers}
	\index{univers!energie@énergie de l’}\index{univers!entropie de l’}
		...
		\thermoquoteend{William Thomson, 1852~\cite{kelvin1852}\index{Kelvin!William Thomson}}{}
		Nous quittons l’entropie sur une question ouverte. Dans la mesure où l’on pense l’univers comme étant un ensemble fini, c’est-à-dire comme à un système isolé contenant une quantité fixe d’énergie, peut-on lui appliquer l’\cref{eq_augmentation_entropie} : $\Delta s_\text{univers} > 0$ lorsque le temps passe ? L’univers tend-il vers une température minimale homogène finale ? Clausius est sans équivoque : il termine aussitôt son article de 1865 par l’affirmation :
			...
			\begin{quote}\index{Clausius!Rudolf}
				Si l’on imagine que l’on ait calculé d’une manière conséquente pour l’univers entier […] la quantité que j’ai nommée \vocab{entropie} pour un corps particulier, ainsi que la quantité désignée sous le nom d’\vocab[energie@énergie!concept]{énergie} et dont le sens est plus facile à saisir, on pourra exprimer très simplement, sous la forme suivante, les lois fondamentales de l’univers qui correspondent aux deux principes essentiels de la théorie mécanique de la chaleur :
					\begin{enumerate}
		...
		\index{Asimov, Isaac}\index{Depondt, Philippe}
