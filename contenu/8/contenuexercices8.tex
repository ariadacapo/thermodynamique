...
\label{exo_diagrammes_ts}
	...
	Représentez, chacune sur un diagramme température-entropie, de façon qualitative et en y représentant éventuellement la courbe de saturation, les évolutions que nous avons déjà étudiées :
...
\label{exo_turbine_vapeur_isentropique}
	...
	\begin{figure}[htp] %handmade
		...
		\end{center}
		\supercaption{Hublot d’inspection d’une des turbines basse pression (puissance \texttildelow\SI{25}{\mega\watt}) du porte-avions \textit{USS Hornet} lancé en 1943.}{\href{https://www.flickr.com/photos/maralinga/15759249479/}{Photo} \ccbysa par Tony Kent}
...
\label{exo_pompe_air}
	...
	\begin{figure}[htp] %handmade
		...
		\end{center}
		\supercaption{Compresseur à air à usage public à Stockholm, destiné aux cyclistes. Un échangeur de chaleur intégré sous la carrosserie permet heureusement d’éviter les températures calculées dans cet exercice.}{Photo recadrée, \wcfile{Cykelpump-Stockholm.jpg}{version originale} \ccbysa \href{https://de.wikipedia.org/wiki/Benutzerin:JakobVoss}{Jakob Voß}}
...
\titreresultats
	...
	\begin{description}
		\item [\ref{exo_questions_cours}]
						...
						\tab 1) Voir \S\ref{ch_entropie_definition} ;
						\tab 2) Oui bien sûr, un simple prélèvement de chaleur suffit : voir à ce propos l’exemple~\ref{exemple_delta_entropie_basics} ;
						\tab 3) Capacité thermique massique : chaleur spécifique $\diff q$ nécessaire pour générer une variation $\diff T$ de température (\cref{eq_def_capacité_calorifique_massique} : $c\ \equiv \ \frac{\diff q}{\diff T}$). Entropie massique : chaleur spécifique $\diff q$ \emph{divisée par la température} à laquelle elle est fournie, pendant une évolution réversible (\cref{eq_variation_franche_entropie}) ;
		...
		\item [\ref{exo_compressions_detentes_irreversibles}]
						\tab 1) $\Delta s_\text{compresseur} = \SI{+1,454}{\kilo\joule\per\kelvin\per\kilogram}$ ; $\Delta s_\text{turbine} = \SI{+0,382}{\kilo\joule\per\kelvin\per\kilogram}$ ;
